// it makes a favourites meal array  in local storage
if (localStorage.getItem("favouritesList") == null) {
    localStorage.setItem("favouritesList", JSON.stringify([]));
}

// it fetches meals from api and return it
async function fetchMealsUsingApi(url,value) {
...
function showMealList(){
    ...
    let html = "";
    let meals=fetchMealsUsingApi(url,inputValue);
    meals.then(data=>{
        if (data.meals) {
...
async function showMealDetails(id) {
    ...
    let html="";
    await fetchMealsUsingApi(url,id).then(data=>{
        html += `
...
async function showFavMealList() {
    ...
    if (arr.length==0) {
        ...
        for (let index = 0; index < arr.length; index++) {
            await fetchMealsUsingApi(url,arr[index]).then(data=>{
                html += `
                <div id="card" class="card mb-3" style="width: 20rem;">
